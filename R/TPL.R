TPL <- function(plant.names = NULL){
    ...
    res1 <- merge(x = res0, y = plantlist.db, by.x = "YOUR_GENUS", by.y = "GENUS", sort = FALSE, all.x = TRUE)
    ...
    return(data.frame(YOUR_SEARCH = res$YOUR_SPECIES, POSSIBLE_GENUS = res$YOUR_GENUS, 
           TPL_STATUS = res$TPL_STATUS, TPL_FAMILY = res$TPL_FAMILY, TPL_ORDER = res$TPL_ORDER, 
           APGIII_NUMBER = res$APGIII_NUMBER, KEW_GENUS_EXT = res$KEW_GENUS_EXT, 
           KEW_FAMILY_EXT = res$KEW_FAMILY_EXT, TPL_URL = res$TPL_URL))
}
